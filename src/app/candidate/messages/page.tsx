...
function ChatPage() {
  ...
  // Subscribe to all conversations for this candidate
  useEffect(() => {
    if (!user) return;
        ...
        <div className="flex-1 overflow-y-auto">
          {filteredConvs.length === 0 ? (
            <div className="flex flex-col items-center gap-2 py-16 text-center text-sm text-muted-foreground px-4">
              ...
              <p className="text-xs">Recruiters who message you will appear here.</p>
            </div>
          ) : (
            filteredConvs.map((c) => {
              ...
              return (
                ...
                >
                  <Avatar className="h-9 w-9 shrink-0">
                    <AvatarFallback className="text-xs">{initials(c.recruiterName)}</AvatarFallback>
                  </Avatar>
      ...
      <div className={cn(
        ...
        {!activeConv ? (
          <div className="flex flex-1 flex-col items-center justify-center gap-3 text-center text-sm text-muted-foreground">
            ...
            <p className="text-xs max-w-xs">Conversations with recruiters will appear on the left.</p>
          </div>
        ) : (
          <>
            {/* Header */}
            <div className="flex items-center gap-3 border-b px-4 py-3">
              ...
              <Avatar className="h-8 w-8">
                <AvatarFallback className="text-xs">{initials(activeConv.recruiterName)}</AvatarFallback>
              </Avatar>
              <div>
                <p className="text-sm font-semibold leading-none">{activeConv.recruiterName}</p>
                <p className="text-xs text-muted-foreground mt-0.5">
                  {activeConv.companyName ?? "Recruiter"}
                </p>
              </div>
            </div>
